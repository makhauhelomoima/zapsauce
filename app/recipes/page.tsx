    ...
    <div className="bg-black min-h-[100dvh] text-white">
      <div className="bg-gray-900/90 border-b border-[#00A651]/40 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
          ...
          <div className="text-xs text-gray-400">MPESA: 57031600 | USSD: *200#</div>
        </div>
      </div>

      <div className="max-w-7xl mx-auto px-4 py-8">
        <h1 className="text-3xl font-black mb-8 text-center">
          <span className="text-[#00E06D]">All 20 Recipes</span>
        </h1>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
          {recipes.map((recipe) => (
            <Link
              ...
              className="bg-gray-900/50 border border-[#00A651]/30 rounded-xl p-4 hover:border-[#00E06D]/50 transition-all"
            >
              <h3 className="font-bold text-white mb-2">{recipe.name}</h3>
              <div className="text-[#00E06D] font-black">
                {recipe.price === 0? 'FREE' : `M${recipe.price}`}
              </div>
              <div className="text-xs text-gray-500 mt-1">Ref: {recipe.id.toUpperCase()}</div>
            </Link>
          ))}
        </div>

        <div className="text-center mt-8">
          <Link href="/" className="text-[#00A651] hover:text-[#00E06D]">← Back to Home</Link>
        </div>
      </div>

      <a
        href="https://wa.me/26657031600?text=Hello%20Zap%20Sauce%20Support%20⚡%20I%20need%20help%20choosing%20a%20recipe"
        target="_blank"
        className="fixed bottom-6 right-6 bg-[#25D366] hover:bg-[#20BA5A] text-white p-4 rounded-full shadow-lg z-50"
      >
