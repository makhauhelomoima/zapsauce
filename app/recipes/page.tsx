  ...
  return (
    <div className="bg-black min-h-[100dvh] text-white p-3">
            ...
            {freeRecipes.map((recipe) => (
              <div key={recipe.id} className="border-2 border-[#00E06D] bg-black/60 backdrop-blur p-5 rounded-xl">
                ...
                <button
                  ...
                  className="bg-[#00A651] hover:bg-[#00C85F] text-white font-black px-6 py-3 rounded-lg text-lg w-full"
                >
                  View Recipe 🎁
                </button>
              </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {paidRecipes.map((recipe) => (
              <div key={recipe.id} className="border-2 border-[#00A651]/60 bg-gray-900/90 backdrop-blur p-5 rounded-xl hover:border-[#00C85F] hover:shadow-xl hover:shadow-green-500/30 transition-all">
                ...
                <ul className="text-sm mb-4 min-h-[80px]">
                ...
                <div className="border-t border-gray-700 pt-4 mt-3">
                  <div className="flex justify-between items-end mb-3">
                    ...
                    <div className="text-right">
                      <div className="text-xs text-[#00C85F] font-bold mb-1">USSD:</div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <button
                      onClick={() => setSelectedRecipe(recipe.id)}
                      className="text-gray-400 hover:text-[#00E06D] text-sm underline"
                    >
                      Preview Ingredients
                    </button>
                  </div>
      ...
      {showEFT && (
        <div className="fixed inset-0 bg-black/95 z-[100] flex items-center justify-center p-4" onClick={() => setShowEFT(null)}>
          <div className="bg-gray-900 border-3 border-[#00A651] rounded-2xl p-6 max-w-md w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-gray-400 mt-4 text-center">WhatsApp proof to 57031600 after payment</p>
      ...
      {selectedRecipe && RECIPES[selectedRecipe] && (
        <div className="fixed inset-0 bg-black/95 z-[100] flex items-center justify-center p-4 overflow-y-auto" onClick={() => setSelectedRecipe(null)}>
          <div className="bg-gray-900 border-3 border-[#00E06D] rounded-2xl p-6 max-w-lg w-full my-8" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-4">
              <h2 className="text-3xl font-black text-[#00E06D]">{RECIPES[selectedRecipe].name}</h2>
              <button onClick={() => setSelectedRecipe(null)} className="text-gray-400 hover:text-white text-4xl leading-none">×</button>
            </div>

            {RECIPES[selectedRecipe].type === 'FREE' || RECIPES[selectedRecipe].isSubscriberOnly? (
              <>
              ...
              <div className="bg-yellow-900/40 border-2 border-yellow-500 rounded-xl p-5 text-center">
                <p className="text-2xl font-black text-yellow-400 mb-3">🔒 LOCKED RECIPE</p>
                <p className="text-white text-lg mb-4">Pay M{RECIPES[selectedRecipe].price} via USSD or EFT to unlock full ingredients + instructions</p>
                {RECIPES[selectedRecipe].ussd && (
                  <button
                    onClick={() => handleUSSD(RECIPES[selectedRecipe].ussd!)}
                    className="bg-[#00A651] hover:bg-[#00C85F] text-white font-black px-6 py-4 rounded-xl text-xl w-full"
                  >
                    📱 Pay {RECIPES[selectedRecipe].ussd}
                  </button>
                )}
              </div>
            )}

            <button onClick={() => setSelectedRecipe(null)} className="text-gray-400 hover:text-white mt-4 w-full text-lg">Close</button>
          </div>
        </div>
      )}
