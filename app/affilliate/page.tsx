...
export default function AffiliatePage() {
  ...
  // TEMP DATA - We'll connect real Supabase later today
  const mockUser = {
    name: 'Makhauhelo',
    earned: 150,
    due: 75,
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
          <div>
            ...
            <p style={{ color: '#888', margin: '4px 0 0 0' }}>Welcome back, {mockUser.name}</p>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '32px' }}>
          <div style={{ background: '#0a0a0a', border: '2px solid #FFD700', borderRadius: '12px', padding: '24px', textAlign: 'center' }}>
            ...
            <p style={{ color: '#FFD700', fontSize: '2rem', margin: 0, fontWeight: 'bold' }}>M{mockUser.earned}</p>
          </div>
          <div style={{ background: '#0a0a0a', border: '2px solid #00ff88', borderRadius: '12px', padding: '24px', textAlign: 'center' }}>
            ...
            <p style={{ color: '#00ff88', fontSize: '2rem', margin: 0, fontWeight: 'bold' }}>M{mockUser.due}</p>
          </div>
          <div style={{ background: '#0a0a0a', border: '2px solid #ff4500', borderRadius: '12px', padding: '24px', textAlign: 'center' }}>
            ...
            <p style={{ color: '#ff4500', fontSize: '2rem', margin: 0, fontWeight: 'bold' }}>{mockUser.rank}</p>
          </div>
        </div>
        ...
        <div style={{ background: '#0a0a0a', border: '2px solid #FFD700', borderRadius: '16px', padding: '32px', marginBottom: '24px' }}>
          ...
          <div style={{ background: '#111', padding: '16px', borderRadius: '8px', marginBottom: '16px' }}>
            <p style={{ color: '#00ff88', fontSize: '1rem', margin: 0, wordBreak: 'break-all' }}>{mockUser.link}</p>
          </div>
          <button onClick={() => { navigator.clipboard.writeText(mockUser.link); alert('Link copied! Share and earn 30% ⚡') }}
        ...
        <div style={{ background: '#0a0a0a', border: '2px solid #00ff88', borderRadius: '16px', padding: '32px' }}>
          ...
          <div style={{ display: 'grid', gap: '12px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid #1a1a1a' }}>
              ...
              <span style={{ color: '#fff', fontWeight: 'bold' }}>{mockUser.clicks}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid #1a1a1a' }}>
              <span style={{ color: '#888' }}>Conversions</span>
              <span style={{ color: '#fff', fontWeight: 'bold' }}>{mockUser.totalSales}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0' }}>
              <span style={{ color: '#888' }}>Your Sponsor</span>
              <span style={{ color: '#00ff88', fontWeight: 'bold' }}>{mockUser.sponsor}</span>
            </div>
          </div>
        </div>
