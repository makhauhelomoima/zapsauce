    ...
    <div className="bg-black min-h-[100dvh] text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {homepageRecipes.map((recipe) => (
            <div key={recipe.id} className="bg-gray-900/50 border border-[#00A651]/30 rounded-xl p-6 hover:border-[#00E06D]/50 transition-all relative">
              {isLocked(recipe.id) && <div className="absolute top-3 right-3 bg-red-900/80 text-white text-xs px-2 py-1 rounded-full font-bold">🔐 LOCKED</div>}
              <div className="flex justify-between items-start mb-3">
                <h3 className="text-lg font-bold text-white">{recipe.name}</h3>
                <div className="text-right">
                  <div className="text-[#00E06D] font-black text-xl">{recipe.price === 0? 'FREE' : `M${recipe.price}`}</div>
              ...
              {isLocked(recipe.id)? (
                <div className="border-t border-gray-700 pt-4">
                  <div className="bg-red-900/20 border border-red-500/40 rounded-lg p-3 mb-3">
                    ...
                    <input type="text" placeholder="Enter MPESA code" value={mpesaCode} onChange={(e) => setMpesaCode(e.target.value)} className="w-full bg-black border border-gray-600 rounded px-3 py-2 text-sm mb-2" />
                    <button onClick={() => verifyPayment(recipe.id)} disabled={verifying} className="w-full bg-[#00A651] hover:bg-[#00C85F] text-black font-bold py-2 rounded text-sm disabled:opacity-50">
                ...
                <div className="border-t border-gray-700 pt-4">
                  ...
                  <Link href={`/recipes/${recipe.id}`} className="block w-full bg-gray-800 hover:bg-gray-700 text-white text-center py-2 rounded text-sm">View Full Recipe</Link>
                </div>
              )}
            </div>
          ))}
        ...
        <div className="text-center">
          <Link href="/recipes" className="inline-block bg-[#00E06D] hover:bg-[#00C85F] text-black font-black px-8 py-4 rounded-lg text-lg">View All 20 Recipes →</Link>
        </div>
